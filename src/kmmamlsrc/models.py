...
class UnetModel(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, chans, num_pool_layers, drop_prob):
        ...
        for i in range(num_pool_layers - 1):
            ...
            ch *= 2
        self.conv = ConvBlockTaskEnc(ch, ch, drop_prob,False)
    ...
    def adaptation(self, input, weights):
        stack = []
        output = input
        #         ch = chans
...
#         print(alphalatent)
    ...
    def adaptation(self,gamma_val, weights):
        ...
        downindex = self.num_pool_layers*2
        
        for i in range(0,self.num_pool_layers):       
            betadown = F.linear(gamma_val,weights[downindex + (2*i)],weights[(downindex + (2*i))+1])
        ...
        latentindex = downindex + self.num_pool_layers*2
        
        alphalatent = F.linear(gamma_val,weights[latentindex],weights[latentindex+1])
        betalatent = F.linear(gamma_val,weights[latentindex+2],weights[latentindex+3])
        latentmodulationweight = torch.matmul(torch.t(betalatent),alphalatent) 
        latentmodulationweight = latentmodulationweight.unsqueeze(2).unsqueeze(3)
#         print(alphalatent.shape, betalatent.shape)
        upindex = 4 + latentindex
        alphauplist = []
        betauplist = []
        ...
        for i in range(0,self.num_pool_layers):       
            betaup = F.linear(gamma_val,weights[upindex + (2*i)],weights[(upindex + (2*i))+1])
            betauplist.append(betaup)
            
        for i in range(0,self.num_pool_layers):  
#             print(alphauplist[i][0][0], betauplist[i][0][0])
